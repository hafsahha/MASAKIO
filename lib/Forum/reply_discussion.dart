...
class ReplyBottomSheet extends StatefulWidget {  
  ...
  final int forumId;
  const ReplyBottomSheet({super.key, required this.onRefresh, required this.forumId});
...
class _ReplyBottomSheetState extends State<ReplyBottomSheet> {
  ...
  Future<void> _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        await addForumReply(widget.forumId, _replyController.text.trim(), _selectedImage);
        if (mounted) {
...
void showReplyBottomSheet(BuildContext context, Function() onRefresh, int discussionId) {
  showModalBottomSheet(
    context: context,
    isScrollControlled: true,
    backgroundColor: Colors.transparent,
    builder: (_) => ReplyBottomSheet(onRefresh: onRefresh, forumId: discussionId),
  );
}
