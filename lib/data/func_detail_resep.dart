...
Future<Resep> fetchDetailResepById(int id) async {
  ...
  return Resep(
    ...
    servings: data['info']['porsi'] ?? 1,
    duration: const Duration(minutes: 0), // Isi jika ada
    price: 0, // Isi jika ada
    nutrition: {
      'Karbohidrat': (data['info']['karbohidrat'] ?? '0').toString() + ' gr',
      'Protein': (data['info']['protein'] ?? '0').toString() + ' gr',
      'Lemak': (data['info']['lemak'] ?? '0').toString() + ' gr',
      'Serat': (data['info']['serat'] ?? '0').toString() + ' gr',
    },
