...
// Function to increment view count when recipe is viewed
Future<void> incrementViewCount(int recipeId) async {
  final client = http.Client();  try {
    final response = await client
        .put(Uri.parse('$recipeUrl/view/$recipeId'))
        .timeout(const Duration(seconds: 5));
    
    if (response.statusCode == 200) {
      print('View count incremented for recipe $recipeId');
    }
  } catch (e) {
    print('Error incrementing view count: $e');
    // Tidak perlu throw error karena ini tidak critical
  } finally {
    client.close();
  }
}
